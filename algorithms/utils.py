...
class ReplayBuffer(RB):
    def __init__(self, buffer_size: int, observation_space: Space, action_space: Space, latent_action_dim: int, device: torch.device | str = "auto", n_envs: int = 1, optimize_memory_usage: bool = False, handle_timeout_termination: bool = True):
    ...
    def add(self, obs: np.ndarray, next_obs: np.ndarray, action: np.ndarray, reward: np.ndarray, done: np.ndarray, infos: List[Dict[str, Any]], latent_action: np.ndarray, previous_latent_action: np.ndarray) -> None:
    ...
    def sample(self, batch_size: int, env: VecNormalize | None = None) -> ExtendedReplayBufferSamples:
        return super().sample(batch_size, env)

    def _get_samples(self, batch_inds: np.ndarray, env: VecNormalize | None = None) -> ExtendedReplayBufferSamples:
